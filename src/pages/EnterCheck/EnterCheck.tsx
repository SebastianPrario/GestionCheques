...
export const EnterCheck: React.FC<EnterCheckProps> = ( { show , onClose} ) => {
  const { Formik } = formik;
  const authContext = useContext(AuthContext)
  const checkContext = useContext(CheckContext)
  const user = authContext && authContext.user
  const addCheck = checkContext && checkContext.addCheck
  const schema = yup.object().shape({
    numero: yup
    .string()
    .matches(/^\d{8}$/, 'El número debe tener exactamente 8 dígitos numéricos')
    ...
    librador: yup
    .string()
    .required('Nombre del Emisor requerido'),
    fechaEmision: yup
    .date().default(() => new Date())
    .test('is-valid-format', 'El formato debe ser aaaa/mm/dd', function(value) {
...
.required('La fecha es obligatoria'),
    fechaEntrega: yup
    .date().default(() => new Date())
    .test('is-before', 'La fecha de pago debe ser posterior o igual a la fecha de emisión', function(value) {
      const { fechaEmision } = this.parent;
      return isEqual(value, fechaEmision) || isAfter(value, fechaEmision)})
    .test('max-difference', 'La diferencia entre la fecha de emisión y la fecha de pago no puede ser mayor a 360 días', function (value) {
          const { fechaEmision } = this.parent;
    ...
    <CustomModal show={show} onClose={onClose}>
     ...
     <Formik
      ...
      onSubmit={( values, { resetForm } )=>{
        values.fechaEmision = format(values.fechaEmision, 'yyyy/MM/dd')
        values.fechaEntrega = format(values.fechaEntrega, 'yyyy/MM/dd') 
        addCheck( values , user?.token )
        resetForm();
      }
      }
      initialValues={{
